...
class Adafruit_SSD1351{
 public:
    Adafruit_SSD1351(uint8_t cd, uint8_t dc, uint8_t reset, uint8_t panel_direction);
    Adafruit_SSD1351(uint8_t cd, uint8_t dc, uint8_t reset);
    void begin(void);
   

  void fillScreen(uint16_t fillcolor);
  void fillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t fillcolor);
  void fillFrame(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *frame_buffer);

 private:
  void writeCommand(uint8_t c);
  void writeData(uint8_t c);
  void rawFillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t fillcolor);
  uint8_t _cs, _dc, _reset,_panel_direction;
};
